...
import AIAssistant from './pages/mobile/AIAssistant';
import Doctors from './pages/mobile/Doctors';
import Appointments from './pages/mobile/Appointments';
import BookAppointment from './pages/mobile/BookAppointment';
import DoctorDashboard from './pages/mobile/DoctorDashboard';
import AdminDashboard from './pages/mobile/AdminDashboard';
import HealthRecords from './pages/mobile/HealthRecords';
...
import Splash from './pages/mobile/auth/Splash';
import Welcome from './pages/mobile/auth/Welcome';
import ForgotPassword from './pages/mobile/auth/ForgotPassword';
import VerifyOTP from './pages/mobile/auth/VerifyOTP';
import ResetPassword from './pages/mobile/auth/ResetPassword';
import RoleSelection from './pages/mobile/auth/RoleSelection';
import SmartHealthHome from './pages/mobile/SmartHealthHome';
import AIChat from './pages/mobile/AIChat';
import SymptomChecker from './pages/mobile/SymptomChecker';
import Reminders from './pages/mobile/Reminders';
...
function ProtectedRoute({ children, role }) {
  ...
  if (loading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-900 font-sans text-white">
        ...
        <h2 className="text-xl font-black text-white tracking-tight mb-2">UniHealth AI</h2>
  ...
  if (role && user.role !== role) {
    return <Navigate to={user.role === 'admin' ? '/admin' : user.role === 'doctor' ? '/doctor' : '/student'} />;
  }
  return children;
}
  ...
  return (
    <Router>
      <NavigationProvider>
        <ToastProvider>
          <div
            className="bg-slate-900 text-slate-100 flex flex-col relative overflow-x-hidden selection:bg-teal-500 selection:text-slate-950"
            style={{ minHeight: '100dvh' }}
          >
            {/* Offline Banner */}
            {!isOnline && (
              <div className="fixed z-[9999] left-4 right-4 bg-rose-600 text-white px-4 py-3 rounded-2xl shadow-2xl flex items-center justify-between border border-rose-500"
                style={{ bottom: 'calc(var(--nav-total) + 12px)' }}
              >
                <div className="flex items-center gap-3 text-sm font-bold">
                  <svg className="w-4 h-4 animate-pulse flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                      d="M18.364 5.636a9 9 0 010 12.728m0 0l-2.829-2.829m2.829 2.829L21 21M15.536 8.464a5 5 0 010 7.072m0 0l-2.829-2.829m-4.243 2.829a4.978 4.978 0 01-1.414-3.536 5 5 0 011.414-3.536m0 0l2.829 2.829m-2.829 4.243L3 21" />
                  </svg>
                  Offline — Check connection
                </div>
                <button
                  onClick={() => setIsOnline(navigator.onLine)}
                  className="text-xs font-black uppercase bg-white/20 px-3 py-1.5 rounded-lg active:scale-95 transition-all"
                >
                  Retry
                </button>
              </div>
            ...
            {/* Main content — offset by header and nav heights */}
            <main
              className="flex-1"
              style={{
            ...
            >
              <AnimatedPageWrapper>
                <Routes>
                  ...
                  <Route path="/student/settings"           element={<ComingSoon title="Settings" />} />
                  <Route path="/student/dashboard-ai"       element={<ComingSoon title="AI Enhanced Dashboard" />} />
                  <Route path="/student/ai-assistant-pro"   element={<ComingSoon title="AI Assistant Pro" />} />
                  <Route path="/student/smart-home-pro"     element={<ComingSoon title="Smart Health Pro" />} />
